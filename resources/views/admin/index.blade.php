...
<body class="antialiased">
        ...
        <aside id="sidebar" class="fixed md:static inset-y-0 left-0 w-72 sidebar-admin flex flex-col transition-transform duration-300 transform -translate-x-full md:translate-x-0 z-30 shadow-xl">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                <a data-page="elections" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl">
                    ...
                    <span class="ml-auto bg-white/20 text-white text-[10px] px-2 py-0.5 rounded-full">{{ $activeElections }} live</span>
                </a>
                <a data-page="candidates" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl">
                    ...
                    <span class="ml-auto bg-white/20 text-white text-[10px] px-2 py-0.5 rounded-full">{{ $candidates->count() }}</span>
                </a>
                <a data-page="voters" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl">
                    ...
                    <span class="ml-auto bg-white/20 text-white text-[10px] px-2 py-0.5 rounded-full">{{ number_format($totalVoters/1000, 1) }}k</span>
                </a>
                ...
                <a href="{{ route('admin.winners') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all">
                    <i class="fas fa-trophy w-6"></i>
                    <span class="ml-3">Winners & Certificates</span>
                </a>
                ...
                <div class="pt-4 mt-6 border-t border-white/10">
                    <a href="{{ route('admin.super-admin.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium rounded-xl group transition-all duration-300 hover:scale-[1.02]">
                        <i class="fas fa-user-shield w-6 text-yellow-400 group-hover:animate-pulse"></i>
                        <span class="ml-3 font-bold text-yellow-50 whitespace-nowrap">Super Admin Panel</span>
                    </a>
                </div>
                @endif
            </nav>
                ...
                <div id="page-dashboard" class="page-content active-page space-y-6">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-gray-500 mt-1">2026 Guild Elections • Presidential, Faculty Reps, Referendum</p>
                            </div>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            @if((\App\Models\SystemSetting::where('key', 'election_ended')->first()->value ?? '0') == '0')
                                <form action="{{ route('admin.finalize-vote') }}" method="POST" onsubmit="return confirm('Attention: This will permanently close all voting and finalize winners. Continue?')">
                                    @csrf
                                    <button type="submit" class="bg-black text-white px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-primary transition-all flex items-center gap-2">
                                        <i class="fas fa-flag-checkered"></i> Finalize Election
                                    </button>
                                </form>
                            @else
                                <span class="bg-green-100 text-green-700 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-widest border border-green-200">
                                    <i class="fas fa-check-double mr-1"></i> Election Finalized
                                </span>
                            @endif
                        </div>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <a href="{{ route('dashboard.live-results') }}" target="_blank" class="flex flex-col items-center p-4 bg-red-50 border border-red-100 rounded-xl hover:bg-red-100 transition cursor-pointer">
                                <i class="fas fa-broadcast-tower text-primary text-xl mb-2 animate-pulse"></i>
    ...
    <script>
        (function() {
            ...
            const lastPage = hash || localStorage.getItem('admin_last_page') || 'dashboard';
            activatePage(pages[lastPage] ? lastPage : 'dashboard');
